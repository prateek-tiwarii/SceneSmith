...
import { Camera, Image, Monitor, Clock, Wand2, Play, Download, RotateCcw } from 'lucide-react';
...
const ScenePreview: React.FC<ScenePreviewProps> = ({ scene, onGenerate }) => {
      ...
      <ScrollArea className="h-full">
        <div className="space-y-6">
          <div className="text-center space-y-2">
            ...
            <div className="flex items-center justify-center gap-2">
              <Badge className={getStatusColor(scene.status)}>
                Scene #{scene.order} • {scene.status.charAt(0).toUpperCase() + scene.status.slice(1)}
              </Badge>
            </div>
          </div>
            ...
            <CardContent className="pt-6">
              <div className="flex gap-2 justify-center">
                ...
                >
                  ...
                  {scene.status === 'pending' ? 'Generating...' : 'Generate Scene'}
                </Button>
                {scene.status === 'failed' && (
                  <Button 
                    variant="outline"
          ...
          <Card className="bg-[#2A2D34] border-[#3A3A3A]">
            ...
            <CardContent>
              <div className="aspect-square bg-[#1A1C22] border-2 border-dashed border-[#3A3A3A] rounded-lg flex items-center justify-center">
                {scene.status === 'pending' ? (
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-green-400 text-sm">Image generated successfully!</p>
                    <p className="text-[#8E8E90] text-xs">Click download to save</p>
                  </div>
                ) : scene.status === 'failed' ? (
                  <div className="text-center">
                    <Camera className="w-12 h-12 text-red-400 mx-auto mb-2" />
                    <p className="text-red-400 text-sm">Generation failed</p>
                    <p className="text-[#8E8E90] text-xs">Try adjusting the prompt</p>
                  </div>
                ) : (
                  <div className="text-center">
                    <Camera className="w-12 h-12 text-[#8E8E90] mx-auto mb-2" />
                    <p className="text-[#8E8E90] text-sm">No image generated yet</p>
                  </div>
                )}
              </div>
            </CardContent>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Badge className="bg-blue-500/20 text-blue-400">#{scene.order}</Badge>
                </div>
                <div>
                  <h3 className="text-sm font-medium text-[#C0C0C0] mb-1">Resolution</h3>
          ...
          <Card className="bg-[#2A2D34] border-[#3A3A3A]">
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <Badge className="bg-[#7D5FF3]/20 text-[#7D5FF3]">{scene.modelUsed}</Badge>
              </div>
              <div>
                <h3 className="text-sm font-medium text-[#C0C0C0] mb-1">Generated Prompt</h3>
              ...
              {scene.negativePrompt && (
                <div>
                  <h3 className="text-sm font-medium text-[#C0C0C0] mb-1">Negative Prompt</h3>
