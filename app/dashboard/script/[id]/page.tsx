...
import { X, Plus, Sparkles, Film, Palette, Tag, Trash2 } from 'lucide-react';
...
const ScriptUpdate: React.FC<ScriptUpdateProps> = ({ onScriptChange }) => {
  ...
  const addTag = (tag: string) => {
    ...
    if (!script.tags.includes(newTag)) {
      updateScript({ tags: [...script.tags, newTag] });
    }
    setTagInput("");
  };
  ...
  const deleteScene = async (sceneId: string) => {
    ...
    try {
      ...
      updateScript({ 
        scenes: script.scenes.filter(scene => scene._id !== sceneId) 
      });
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await axios.put(`/api/project/${id}`, script);
